...
### 🚨 Slashed Orders — Почему происходят и как избежать

| Причина                        | Описание                                                                                  | 💡 Решение                                                                                   |
|-------------------------------|--------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------|
| ❌ Неуспешный proof            | Ордер был залочен, но proof не был отправлен                                              | Убедись, что `proof_retry_count` и `proof_retry_sleep_ms` адекватны, агент не падает       |
| 🕒 Превышен дедлайн            | Ордер начал выполнение, но не успел завершиться до истечения времени                      | Уменьши `max_concurrent_proofs` или `peak_prove_khz`, чтобы не брать слишком много заказов |
| ⚠️ Невалидный proof           | Proof отклонён, так как он некорректен                                                     | Проверь версии zkVM (например, `bonsai_r0_zkvm_ver`) и стабильность агента                 |
| 🧨 Контейнер упал             | Брокер или агент остановился, не завершив заказ                                           | Используй `watchtower` или systemd/tmux для автоперезапуска                                |
| 📦 Журнал слишком большой      | Вычисления сгенерировали `journal` превышающий лимит `max_journal_bytes`                 | Увеличь `max_journal_bytes` в `broker.toml`, если уверен, что сможешь завершать такие заказы |
